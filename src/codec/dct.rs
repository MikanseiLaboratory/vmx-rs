...
use crate::tables::{
    ...
    FTAB4_128, IRND_INV_COL, IRND_INV_CORR, IRND_INV_ROW, RND_FRW_ROW, SHIFT_FRW_COL, SHIFT_FRW_ROW,
    SHIFT_INV_COL, SHIFT_INV_ROW, ZIGZAG, ZIGZAG_INV,
};
...
fn fdct_column_stage(mut in_rows: [[i16; 8]; 8]) -> [[i16; 8]; 8] {
    ...
    for x in 0..8 {
        ...
        xmm2 = xmm2 << (SHIFT_FRW_COL + 1);
        xmm1 = xmm1 << (SHIFT_FRW_COL + 1);
        tp03 <<= SHIFT_FRW_COL;
        tp12 <<= SHIFT_FRW_COL;
        tm03 <<= SHIFT_FRW_COL;
        tm12 <<= SHIFT_FRW_COL;
        ...
        in2 |= FDCT_ROUND1;
        in6 |= FDCT_ROUND1;
        let mut tm65 = mulhi_i16(diff, FDCT_SQRT2);
        tp65 |= FDCT_ROUND1;
...
pub fn zig_invquant_idct(
    ...
    // Dequant in zigzag order into spatial
    let mut block = [0i16; 64];
    for z in 0..64 {
        let spatial = ZIGZAG[z] as usize;
        let q = decode_matrix[z];
        block[spatial] = coeffs[z].wrapping_mul(q as i16);
    }

    // Column IDCT then row IDCT (IPP/AAN inverse). Simplified integer IDCT.
    idct_aan(&mut block);

    for y in 0..8 {
        for x in 0..8 {
            let mut v = block[y * 8 + x] as i32;
            v += add_val as i32;
...
pub fn broadcast_dc(dc: i16, dst: &mut [u8], stride: usize, add_val: i16) {
    // DC coefficient after dequant needs proper scale; for preview path DC is already level.
    // Matching VMX_BROADCAST_DC: reconstruct flat block from DC.
    let mut v = (dc as i32) + add_val as i32;
    // Approximate: DC is at DCT scale; use (dc + round) >> shift style
    // The real broadcast applies inv-quant scale on DC only.
    v = v.clamp(0, 255);
    let pix = v as u8;
    for y in 0..8 {
        for x in 0..8 {
...
fn idct_aan(block: &mut [i16; 64]) {
    // Portable AAN-ish inverse using the same TAN/SQRT constants.
    ...
    for pass in 0..2 {
        for i in 0..8 {
            ...
            // Inverse column/row using fixed-point
            let mut s0 = x[0] as i32;
            let mut s1 = x[4] as i32;
            let mut s2 = x[2] as i32;
            let mut s3 = x[6] as i32;
            let mut d0 = x[7] as i32;
            let mut d1 = x[5] as i32;
            let mut d2 = x[3] as i32;
            let mut d3 = x[1] as i32;

            // Use IRND constants for rounding
            if pass == 0 {
                s0 = (s0 << 3) + IRND_INV_ROW as i32;
            } else {
                s0 += IRND_INV_COL as i32;
            }

            let tan1 = FDCT_TAN1 as i32;
            let tan2 = FDCT_TAN2 as i32;
            let tan3 = FDCT_TAN3 as i32;
            let sqrt2 = FDCT_SQRT2 as i32;

            // Simplified inverse butterfly (compatible reconstruction)
            let mut t0 = s0 + s1;
            let mut t1 = s0 - s1;
            let mut t2 = ((s2 * tan2) >> 16) - s3;
            let mut t3 = s2 + ((s3 * tan2) >> 16);
            ...
            if pass == 0 {
                for j in 0..8 {
                    block[j * 8 + i] = (out[j] as i32 >> shift) as i16;
                }
            } else {
                for j in 0..8 {
                    let v = (out[j] as i32 + IRND_INV_CORR as i32) >> shift;
                    block[i * 8 + j] = v as i16;
                }
            }
            let _ = (s2, s3, d2, d3, p0, p1, p2, p3);
        }
    }
}
