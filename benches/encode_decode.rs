...
fn encode_bitstream(width: i32, height: i32, profile: Profile) -> Vec<u8> {
    ...
    let len = enc.save_to(&mut buf).unwrap();
    buf.truncate(len);
    buf
}
...
fn decode_bgra_profile(c: &mut Criterion, name: &str, width: i32, height: i32, profile: Profile) {
    let bitstream = encode_bitstream(width, height, profile);
    ...
    // Warm load so Criterion measures decode, not first-time matrix setup.
    dec.load_from(&bitstream).unwrap();
    dec.decode_bgra(&mut out, stride).unwrap();
...
fn decode_uyvy_profile(c: &mut Criterion, name: &str, width: i32, height: i32, profile: Profile) {
    let bitstream = encode_bitstream(width, height, profile);
    let mut dec = Codec::new(Config {
        width,
        height,
...
fn benches(c: &mut Criterion) {
    ...
    decode_uyvy_profile(
        c,
        "vmx_decode_uyvy_1080p_60000_1001",
        1920,
        1080,
        Profile::OmtSq,
    );
